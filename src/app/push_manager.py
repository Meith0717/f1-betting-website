...
class PushManager:
    ...
    def _ensure_data_file_exists(self):
        ...
        try:
            ...
            self._logger().info("Created new push subscriptions file: %s", self.data_file)
        ...
        try:
            ...
            if backup_file.exists():
                try:
                    ...
                    self._logger().error(
                        "Failed to restore backup after save failure: %s", 
                        restore_error
                    )
            raise
    ...
    def add_subscription(
        ...
        try:
            ...
            existing = next(
                (s for s in data["subscriptions"][username] 
            ...
            if existing:
                ...
                self._logger().debug(
                    "Updated existing subscription for %s", username
                )
            else:
                ...
                self._logger().info(
                    "Added new push subscription for %s", username
                )
    ...
    def remove_subscription(self, username: str, endpoint: str) -> bool:
        ...
        try:
            ...
            if len(data["subscriptions"][username]) < original_count:
                ...
                self.save_subscriptions(data)
                self._logger().info(
                    "Removed push subscription for %s", username
                )
                return True
            ...
            self._logger().error(
                "Error removing subscription for %s: %s", username, e
            )
            return False
    ...
    def remove_all_subscriptions(self, username: str) -> int:
        ...
        try:
            ...
            self.save_subscriptions(data)
            self._logger().info(
                "Removed %s push subscriptions for %s", count, username
            )
            return count
    ...
    def get_user_subscriptions(self, username: str) -> List[Dict]:
        ...
        try:
            ...
            self._logger().error(
                "Error getting subscriptions for %s: %s", username, e
            )
            return []
    ...
    def count_subscriptions(self) -> int:
        ...
        try:
            data = self.load_subscriptions()
            return sum(
                len(subs) for subs in data.get("subscriptions", {}).values()
            )
        except Exception as e:
