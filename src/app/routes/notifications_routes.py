    ...
    try:
        ...
        if success:
            current_app.logger.info(
                f"Push subscription saved for user: {username}"
            )
            return jsonify({
                'success': True,
...
def remove_subscription():
    ...
    try:
        ...
        username = session.get('username')
        if not username:
            current_app.logger.warning("No username in session for subscription removal")
            return jsonify({
                'success': False,
                'error': 'Not logged in'
            }), 401
        ...
        if success:
            current_app.logger.info(
                f"Push subscription removed for user: {username}"
            )
            return jsonify({
                'success': True,
...
def remove_all_subscriptions():
    ...
    try:
        ...
        current_app.logger.error(f"Error removing all subscriptions: {e}", exc_info=True)
        return jsonify({
            'success': False,
            'error': str(e)
        }), 500
...
def _get_vapid_keys():
    ...
    if public_key and private_key:
        ...
        current_app.logger.error("VAPID keys NOT loaded - public_key: %s, private_key: %s", 
                                   bool(public_key), bool(private_key))
...
def send_push_notification(username, title, message, data=None, url=None):
    ...
    try:
        ...
        if not subscriptions:
            current_app.logger.warning(f"No push subscriptions found for user: {username}")
            return {
                'success': True,
                'sent': 0,
                'failed': 0,
        ...
        for subscription in subscriptions:
            try:
                ...
                current_app.logger.error(
                    f"VAPID signature error for {username}: {e}"
                )
                failed += 1
                errors.append({
                    'endpoint': subscription['endpoint'],
                ...
                current_app.logger.error(
                    f"HTTP error sending to {username}: {e}"
                )
                failed += 1
                errors.append({
                    'endpoint': subscription['endpoint'],
                ...
                current_app.logger.error(
                    f"Error sending push to {username}: {e}"
                )
                failed += 1
                errors.append({
                    'endpoint': subscription['endpoint'],
        ...
        current_app.logger.error(f"Fatal error in send_push_notification: {e}", exc_info=True)
        return {
            'success': False,
            'error': str(e),
            'sent': 0,
...
def admin_send_test():
    ...
    try:
        ...
        title = request.form.get('title') or request.json.get('title', 'Test Notification')
        message = request.form.get('message') or request.json.get('message', 'This is a test push notification')
        ...
        if result['success'] or result['sent'] > 0:
            flash(f"Test notification sent to {username}! Sent: {result['sent']}, Failed: {result['failed']}", 'success')
        else:
...
def admin_send_to_all():
    ...
    """
    try:
        title = request.form.get('title') or request.json.get('title', 'Important Update')
        message = request.form.get('message') or request.json.get('message', 'Check out the latest F1 news!')
